...
can be done in two $O(\log n)$ binary searches. On the first glance this should
be a lot faster than the na\"ive $O(n)$ scan, but a binary search has a number
of downsides.
...
\begin{itemize}
  ...
  \item A binary search executes $O(\log n)$ branch instructions, every single
    branch is dependent on the input data and not predictable. Mispredicted
    branches slow down the scan as the processor has to wait on the result
    before proceeding with the next value.
...
distribution and the percentage of values selected during the scan, so this is
